...
public class TeacherHomeController implements User {
    ...
    public boolean resetPassword(String password) throws SQLException {
        ...
        statement= connection.prepareStatement("UPDATE Teachers SET password=SHA1(?) WHERE id=?");
    ...
    public static boolean addNewTeacher(Teacher tt) throws SQLException {
        boolean status;
        Connection connection = new DButil().getConnection();
        // Prepare Statement
        PreparedStatement st  = connection.prepareStatement("INSERT INTO Teachers(name,email,password) VALUES (?,?,SHA1(?))");
        st.setString(1, tt.getName());
